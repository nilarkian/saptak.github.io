---
layout: timeline
title: "Tech News Tracker"
last_updated: "April 2026"
---

## April 2026

### 🌞 Intel Bets on Self-Assembling Materials to Salvage Moore's Law

**27th April 2026** · Semiconductors

The global semiconductor race is shifting from a battle of machines to a battle of molecular chemistry. For 50+ years the industry shrunk transistors using finer beams of light — but as chip features approach atomic scale, the $200M EUV machines powering today's fabs are hitting a physical wall. The brush of light is simply too thick for the designs it needs to draw.

Enter **Directed Self-Assembly (DSA)** — block copolymers that naturally organise into precise, repeating patterns when heated. A coarse EUV template guides them, and the molecules do the rest — producing features sharper and smaller than the wavelength of light itself.

The industry is split. Sony already uses DSA for image sensors. Intel is betting on it for its high-stakes **14A node by 2027** — a direct attempt to bypass the brute-force economics of next-gen EUV machines. TSMC and Samsung are staying machine-heavy for now.

If Intel's gamble lands, the next era of computing won't be defined by the machines we build — but by materials engineered to build themselves.

[Source →](https://newsletter.semianalysis.com/p/intels-14a-magic-bullet-directed)


---

## October 2025

### 🌍 India Locks In Its First Binding FDI Pledge — With  Four Nordic Countries
**1st October 2025** · Policy · India

India's trade agreement with EFTA — Iceland, Liechtenstein, Norway, Switzerland — entered into force today. The headline isn't the tariffs. It's the **USD 100 billion FDI commitment over 15 years**, the first binding investment pledge in any Indian FTA, directly linked to **1 million direct jobs**. EFTA covers **92.2% of India's export basket** with near-zero duties. India reciprocates on **82.7% of tariff lines**, phased over 10 years — gold, machinery, and pharma included. Dairy stays protected.

The deal is a geopolitical hedge as much as a trade play. But USD 100 billion from four small economies is an ambitious ask — Switzerland accounts for most of EFTA's financial firepower, and pledges on paper don't build factories.

[Source →](https://www.pib.gov.in/PressReleasePage.aspx?PRID=2177724)

---

## May 2025

### 🚀 ULI Hits ₹65,000 Crore — India's Lending Stack Has a New Engine

**May 2025** · Fintech · India

India's Unified Lending Interface just posted numbers that are hard to ignore. **1.4 million loans totaling ₹65,000 crore** disbursed by April 2025 — up from ₹27,000 crore across 600,000 loans just five months earlier in December 2024. Monthly disbursement rate nearly tripled.

The headline product is the **Kisan Credit Card loan** — what previously took 30+ days across multiple bank visits now clears in under 15 minutes. 40% of all disbursements are going to farmers. 44 lenders are onboard including SBI, NBFCs, regional rural banks and fintechs. Lender registrations are up 300% since January 2025.

RBI Governor has called ULI a potential **"UPI for lending."** If that comparison holds, India just laid the rails for the next decade of credit.

[Source →](https://www.financialexpress.com/business/banking-finance/uli-facilitated-over-1-4-million-loans-worth-rs-65k-crore-says-rbi/3818576/)

---

### ⚙️ ULI's Real Innovation Isn't the Loans — It's the Plumbing

**May 2025** · Fintech · Infrastructure

The numbers are impressive but the architecture is the actual story. ULI runs on **plug-and-play APIs pulling from 60+ data sources** — land records, Aadhaar, credit bureaus, GST, utility payments, dairy herd data, satellite imagery, groundwater levels. A lender plugs in once and gets access to all of it, eliminating bilateral integrations entirely.

Consent is managed via **blockchain-based audit logs** with differential privacy layered on top. Borrowers approve data access explicitly; everything is traceable. Five Account Aggregators handle the financial data pipeline. Tamil Nadu even feeds in groundwater levels so lenders can price drought risk into agricultural loans.

The technical complexity reduction is estimated at **70%** versus the old model of lenders stitching together data partnerships one by one. This is what makes 15-minute loan approvals structurally possible — not just a product feature, but a consequence of the stack.

[Source →](https://www.drishtiias.com/daily-updates/daily-news-analysis/rbi-to-launch-unified-lending-interface)

---

### ⚠️ ULI Has Real Problems That the Growth Numbers Are Hiding

**May 2025** · Fintech · Analysis

The pitch is compelling. The cracks are real. Three problems worth watching:

**Lender margins are brutal.** MSME loans under ₹10 lakh carry servicing costs of 8–12%, leaving net interest margins below 2%. Public sector banks dominate because they're mandated to hit priority-sector targets. Private banks are largely sitting out — the economics don't work for them yet.

**The data gap is structural.** 35% of rural borrowers have no formal credit history. Only 22% of MSMEs are GST-registered, which is a hard ceiling on how much of the market ULI can actually score. Fintechs are falling back on proxy signals like mobile recharge patterns — unproven as reliable underwriting data.

**Regulation is contradicting itself.** ULI is designed as an open platform. But RBI's stricter capital adequacy norms for fintech lenders are functionally locking out the players who would scale it fastest. The result risks a two-tier system where traditional banks dominate and the "open" promise never fully materialises.

[Source →](https://www.medianama.com/2025/06/223-india-unified-lending-interface-privacy-risks-inclusive-credit/)

---

### 🎯 The Government's Bet: ₹5,000 Crore to Make ULI Inevitable

**May 2025** · Fintech · Policy

The RBI and government aren't waiting for market forces to solve ULI's adoption problem — they're writing cheques. Two interventions on the table:

**₹1,200 crore** earmarked for FY2025–26 to subsidise MSME GST registration and farmer land record digitization — directly attacking the data gap that limits credit scoring.

**₹5,000 crore default guarantee fund** proposed to de-risk private bank participation. If it passes, it removes the single biggest reason private banks are sitting out.

The target: **₹2 trillion in annual disbursements by 2026.** Whether that's achievable depends on whether these interventions move fast enough to align lender incentives with the platform's ambitions. Infrastructure is built. The economics still need fixing.

[Source →](https://economictimes.indiatimes.com/news/economy/policy/rbi-to-review-digital-banking-regulations-expand-lending-interface-and-cbdc-pilots/articleshow/121494735.cms)